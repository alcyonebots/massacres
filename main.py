...
async def generate_invite_link(bot: Bot):
    ...
    try:
        ...
        await send_log(bot, f"**[New Link]** Valid for {invite_expiry} min: `{current_invite_link}`")
        ...
        await send_log(bot, "**[Updated]** Inline button refreshed.")

    except Exception as e:
        await send_log(bot, f"**[Error]** Updating invite link: `{e}`")
...
async def set_expiry_time(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await send_log(context.bot, f"**[Config]** Admin {update.effective_user.mention_html()} set expiry to {invite_expiry} minutes.")

        await generate_invite_link(context.bot)  # Generate a new link after updating expiry

    except Exception as e:
        await update.message.reply_text("**Error processing your request.**", parse_mode="Markdown")
        await send_log(context.bot, f"**[Error]** Setting expiry time: `{e}`")
